...
REASON_CODE_SKILL_GROUP_DENIED = "REASON_CODE_SKILL_GROUP_DENIED"
...
def unified_policy_gate(ctx: PolicyContext, actx: RuntimeActionContext, profile: PolicyProfile) -> PolicyDecisionEnvelope:
    ...
    # 3.5) skill_group profile gate
    # skill_group is not a second action name. It is the operational lane used by
    # profiles to enable/disable broad capability families without duplicating every action_type.
    skill_group = (actx.skill_group or "generic").strip() or "generic"
    if skill_group in set(profile.denied_skill_groups):
        return PolicyDecisionEnvelope(
            decision_code=DecisionCode.DENY,
            primary_reason_code=REASON_CODE_SKILL_GROUP_DENIED,
            effective_scope=ctx.scope,
            effective_profile_id=profile.name,
            effective_risk_level=actx.risk_level,
            policy_trace_id=f"policy-{actx.task_id}",
            audit_tags=["policy", "deny", "skill_group"],
        )
    if profile.allowed_skill_groups and skill_group not in set(profile.allowed_skill_groups):
        return PolicyDecisionEnvelope(
            decision_code=DecisionCode.DENY,
            primary_reason_code=REASON_CODE_SKILL_GROUP_DENIED,
            effective_scope=ctx.scope,
