...
def validate_scene(scene: dict[str, Any]) -> SceneSummary:
    ...
    for i, vehicle in enumerate(vehicles):
        p = vehicle["initial_pose"]
        for other in vehicles[i + 1:]:
            q = other["initial_pose"]
            if math.hypot(float(p["x_m"])-float(q["x_m"]), float(p["y_m"])-float(q["y_m"])) < 5.0:
                raise SceneValidationError("vehicle spawn separation must be >= 5m")
